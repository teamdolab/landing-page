-- ============================================
-- 랜딩페이지 Supabase 스키마 (RLS 없는 개발용 버전)
-- ============================================
...
COMMENT ON COLUMN user_info.pin IS '전화번호 뒷 4자리 (자동 생성)';
COMMENT ON COLUMN user_info.credits IS '사용 가능한 크레딧 (마케팅 동의 3000원, 추천인 2000원)';
...
COMMENT ON TABLE sessions IS '게임 세션 정보 테이블';
COMMENT ON COLUMN sessions.session_id IS '세션 ID (YYMMDD + 매장코드 + 시즌 + 게임타입 + 타임슬롯)';
COMMENT ON COLUMN sessions.current_capacity IS '현재 참가 인원 (자동 계산)';
...
COMMENT ON TABLE apply IS '게임 참가 신청 정보 테이블';
COMMENT ON COLUMN apply.used_credits IS '사용한 크레딧 금액';
COMMENT ON COLUMN apply.final_price IS '최종 결제 금액 (base_price - used_credits)';

-- ============================================
-- 4. 자동화 함수 및 트리거
-- ============================================

-- 4-1. 업데이트 시각 자동 갱신 함수
...
-- 4-3. 참가 신청 시 세션 참가 인원 증가 및 상태 업데이트 함수
...
BEGIN
    ...
    RAISE NOTICE '  - user_info (유저 정보)';
    RAISE NOTICE '  - sessions (게임 세션)';
    RAISE NOTICE '  - apply (참가 신청)';
    RAISE NOTICE '';
    RAISE NOTICE '자동화 기능:';
    RAISE NOTICE '  ✓ 가입 시 크레딧 자동 적립';
    RAISE NOTICE '  ✓ 신청 시 세션 정원 자동 관리';
    RAISE NOTICE '  ✓ 신청 시 크레딧 자동 차감';
    RAISE NOTICE '  ✓ 취소 시 크레딧 자동 복구';
    RAISE NOTICE '';
